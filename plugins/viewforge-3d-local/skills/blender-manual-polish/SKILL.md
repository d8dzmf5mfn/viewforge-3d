...
If the requested change can be expressed as a bounded automated vertex smooth, use the local
`smooth_model_surface` tool and its QA artifacts first. Use this manual workflow when brush-level
visual judgment is genuinely required; do not describe the automated tool as a manual sculpt.
