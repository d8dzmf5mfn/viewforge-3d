...
In ViewForge 3D Local, prefer the `smooth_model_surface` MCP tool for an ordinary Blend or GLB.
Pass either a workspace path or an existing `asset_`/`artifact_` ID. Limit the edit with
`object_names` or a reviewed Blender `vertex_group` whenever the whole model is not authorized.
Keep `preserve_volume` and `preserve_boundaries` enabled by default, and set an explicit
`max_displacement_ratio` budget. Poll `get_viewforge_job`, read `smoothing-qa.json`, then use
`get_local_artifact_path` and `render_model_preview` for local inspection. The source is immutable;
the tool must return a new Blend and GLB.
